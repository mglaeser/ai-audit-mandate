# Volume I — Foundation and Core Tracks
## Due-Diligence and Remediation Mandate · Part 1 of 2 — Core Regime, Product & Platform Tracks, and the Standing Constitution
### For a web application implemented end-to-end by AI, maintained without human code review, and required to stay correct

> **Volume I of II.** Establishes the apparatus: rules of engagement, substitution principles, severity bands, the
> execution protocol, the finding-record schema, Tracks A and B (79 checks), the standing regime, and the Constitution.
> Run this volume to closure first. [Volume II](02-security-privacy-assurance.md) carries Track C and executes after.
...
**Part 1 is standalone-complete; it is not the whole mandate.** Everything this volume produces is final for its scope and built to be extended, never reopened: the catalogue manifest registers Track C's 40 checks from day one as a planned extension; the doors in §6.5 are sized for the Track C rooms they will also close; the regime's ratchet register carries Track C's metric slots from founding. The one thing Part 1 alone can never produce is a **production clearance** — Track C holds two of this catalogue's three unconditional `STOP-SHIP` checks (`C-01`, `C-04`; the third, `B-06`, this volume closes), and no volume that has not audited them is entitled to clear traffic past them. That constraint is not a sentence a reader can skim; it is a computed field (`production_eligible: false`) in `audit/engagement-status.json` that a deploy gate reads and fails closed on (§8).
...
- **Probe:** **Attempt exfiltration.** Plant instructions in content the system will retrieve, directing the model to encode data into an outbound URL, a rendered image source, a markdown link, an email body, or a created pull request — and into whatever else this system can reach: a webhook, a DNS lookup, a telemetry field, a log line something ships onward. **Enumerate the channels first, then attack each one.** A channel you never listed is a channel you never probed, and an unprobed channel is indistinguishable from an absent one in the report. Then check both halves: **was it blocked automatically, and was the session contained automatically?** An alert to a human is not containment. In this system, an alert to a human is a log line with a delivery cost.
...
| `governance/constitution.md` *(in the repository, not in `audit/`)* | **Appendix A, instantiated** — committed at Phase 4 in state `IN_FORCE_PROVISIONAL` beside the two-volume mandate (`governance/mandate/01-foundation-and-core-tracks.md`, `governance/mandate/02-security-privacy-assurance.md`, `governance/mandate/manifest.json`, and the generated canonical concatenation `governance/mandate.md`), **ratified at this volume's Phase 7** (`RATIFIED`, catalogue v1.0, Track C register slots `pending-baseline: part2`), amended and re-ratified at Part 2's Phase 7 (v2.0), hash-attested — constitution, mandate manifest and combined mandate alike — write-separated (B-35), declared by every agent session. **The one artifact every future agent will actually load.** |
...
**The one line this volume may never write: that the system is cleared to serve production traffic.** Track C carries two of the catalogue's three unconditional `STOP-SHIP` checks — `C-01` (core application security) and `C-04` (privacy and data governance); the third, `B-06`, this volume closes — plus `C-06`, which joins them if any model here can call a tool that writes, sends, spends, deletes or executes. They are registered, counted, and unaudited — and to the deploy gate, an unaudited `STOP-SHIP` scope reads exactly like an open `STOP-SHIP` finding. The machine you are leaving behind will keep Tracks A and B true indefinitely; take the executive summary's scope banner seriously, hand over, and schedule Part 2.
...
- **Experimental.** Exempt from every article except the non-negotiables: no secrets, no production credentials, no personal data, egress confined to a sandbox allowlist, **and it publishes nothing that runs on anyone else's machine**. The exemption is safe by construction, not by trust — the credential broker cannot mint production credentials for this class, and the network cannot reach production from it. Note what the first four conditions test: whether production is reachable *from* this repository. They say nothing about what leaves it. A repository that ships a package, an image, a binary or a firmware build to third parties has already reached production — someone else's — and is **Incubating** at minimum, however isolated its own runtime is. Freedom here is structural isolation, not permission.
